...
//此类用于生成  违纪行为分类情况表 的政纪处分部分
public class SqlMaker8 {
	 public static String maker(String start,String end,String fields){
	    	...
	    	sb.append(" FROM(SELECT c.blResult_zjcf AS blResult_zjcf,sum(c.A) AS A,sum(c.B) AS B,sum(c.C) AS C,");
	    			sb.append(" sum(c.D) AS D,sum(c.E) AS E,sum(c.F) AS F,sum(c.G) AS G ,sum(c.H) AS H ,sum(c.I) AS I  ");
	    	sb.append(" FROM(SELECT IF (LOCATE('0118', pu.cfResult) = 0 ,0,SUBSTRING(pu.cfResult,LOCATE('0118', pu.cfResult),6)) AS blResult_zjcf,");
	    	...
	    	sb.append(" FROM problemclues p join  zyviolation z on z.reflectedId=p.reflectedPersonId  join punishment pu on pu.reflectedId=p.reflectedPersonId join progress pro on pro.causeId=p.id and pro.pointValue=25   WHERE p.receiveDate BETWEEN '"+start+"' AND '"+end+"' "+fields+") c GROUP BY c.blResult_zjcf) t");
	    	sb.append(" RIGHT JOIN dict d ON d.value= t.blResult_zjcf WHERE d.dictPid = '0118'");
