...
public class SqlMaker1 {
     ...
     public static String makeCommon(String start,String end,String fields){
         ...
         sb.append(" p.beReflectRank AS be,");
   		 sb.append("IF (LOCATE(1, p.fyZyViolation) = 0 OR LOCATE(1, p.fyZyViolation) IS NULL,0,1) AS A,");
   		 sb.append("IF (LOCATE(2, p.fyZyViolation) = 0 OR LOCATE(2, p.fyZyViolation) IS NULL,0,1) AS B,");
         sb.append("IF (LOCATE(3, p.fyZyViolation) = 0 OR LOCATE(3, p.fyZyViolation) IS NULL,0,1) AS C,");
         sb.append("IF (LOCATE(4, p.fyZyViolation) = 0 OR LOCATE(4, p.fyZyViolation) IS NULL,0,1) AS D,");
         sb.append(" IF (LOCATE(5, p.fyZyViolation) = 0 OR LOCATE(5, p.fyZyViolation) IS NULL,0,1) AS E,");
         sb.append(" IF (LOCATE(6, p.fyZyViolation) = 0 OR LOCATE(6, p.fyZyViolation) IS NULL,0,1) AS F,");
         sb.append(" IF (LOCATE(7, p.fyZyViolation) = 0 OR LOCATE(7, p.fyZyViolation) IS NULL,0,1) AS G ");
         sb.append("FROM");
         sb.append(" problemclues p  where p.receiveDate BETWEEN '"+start+"' and '"+end+"' "+fields+") t GROUP BY t.be");
