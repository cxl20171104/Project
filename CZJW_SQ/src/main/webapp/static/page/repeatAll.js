...
$(function() {
        ...
        dataGrid =	$('#dataGrid').datagrid({
            ...
            pagination : true,
            singleSelect : true,
            queryParams: { pageName: $("#pageName").val(),zddb:$("#zddb").val(),id:$("#cluesId").val(),reflectedName:$("#reflectedName").val()},    
            selectOnCheck : false,
            checkOnSelect : false,
            border : false,
            ...
            columns : [ [ {
            	...
            	width : '120',
            	formatter:function(value,row,index){ 
            		return '<button onclick="addRepeat('+index+');">建立重复关系</button>'
            	}
            },{//（新添加）
            	title:"操作",
            	field:"work",
...
//重复件操作列表
function addRepeat(index){
	  var rows = $('#dataGrid').datagrid('getRows');
	  var row = rows[index];
	  //explan:要加入的案件的id
	  var id = row.id;
	  //explan:被加入案件的id
	  var inId=$("#cluesId").val();
	  $.getJSON(path+"/repeat/addRepeat?id="+id+"&inId="+inId,function(req){
		     $.messager.alert('我的消息',req.msg,'info',function(){
			  refreshTable();
		     });
      });	  
}