        ...
        dataGrid =	$('#dataGrid').datagrid({
            ...
            columns : [ [ {
            	...
            	formatter:function(value,row,index){   
		        	...
		        	return '<button style="color:white;" onclick="get(\''+row.id+'\',\''+row.fromId+'\',\''+$("#organId").val()+'\');">签收</button>';
            	}
            },{
            	title:"查看详情",
            	field:"checkDetail",
            	formatter:function(value,row,index){   
		        	return '<button style="color:white;" onclick="checkDetail(\''+row.id+'\',\''+row.reflectedName+'\');">查看详情</button>';
            	}
            },{//（新添加）
            	title:"退回",
            	field:"returnClues",
            	formatter:function(value,row,index){   
		        	...
		        	return '<button style="color:white;" onclick="returnClues(\''+row.id+'\',\''+row.reflectedName+'\');">退回</button>';
            	}
            }] ],
...
function get(id,fromId,organId){
	$.messager.confirm('提醒', '确认签收？', function(r){
		if(r){
			console.log(fromId);
			$.ajax({
				url:path+"/probleClues/get",
				data:{id:id,fromId:fromId,organId:organId},
				dataType:'json',
				type:'post',
				success:function(result){
					if(result.success){
...
function checkDetail(id,reflectedName){
	 parent.addTab("详情："+reflectedName,path+"/probleClues/action?id="+id+"&pageName="+$("#pageName").val());
}
//（新添加）
