            ...
            columns : [ [ {
            	...
            	formatter:function(value,row,index){   
            		if($("#pageName").val()=="having"||$("#pageName").val()=="newClues"||$("#pageName").val()=="initial"||$("#pageName").val()=="returnList"){
            			...
            			return '<button style="color:white;" onclick="action(\''+row.reflectedName+'\',\''+row.id+'\');">详情</button>&nbsp;<button style="color:white;" onclick="del_reason('+index+');">删除理由</button>';
            		}
            	}
            }] ],
...
function deleteFun(){
   ...
   $('#delete_reason_dia').dialog('open');
}
function cancle_it(){
   $('#delete_reason_dia').dialog('close');
}
function delete_it(){
	...
	if(rows.length>0){
				...
				$.ajax({
					url:path+'/ags/del',
					data:{ids:ids.join(),reason:$("#delete_reason").val()},
					dataType:'json',
					async : true,
					type:'post',
...
function del_reason(index){
      ...
      parent.$.modalDialog({
          ...
          width : 880,
          height : 370,
          href : path+'/probleClues/del_reason?causeId='+row.id
      });
