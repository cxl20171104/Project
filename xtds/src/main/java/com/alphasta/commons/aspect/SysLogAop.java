...
public class SysLogAop {
	...
	public Object recordSysLog(ProceedingJoinPoint point) throws Throwable {
		...
		String param=bfParams.toString();
        if(param.length()>1000){
        	param=param.substring(0, 1000);
        }
		String strMessage = String.format("[类名]:%s,[方法]:%s,[参数]:%s",
				strClassName, strMethodName,param);
		...
		if (isWriteLog(strMethodName)) {
			try {
				...
				if (null != collection) {
					String loginName = collection.getPrimaryPrincipal()
							.toString();
